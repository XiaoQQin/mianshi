...
### 5.1 关于ArrayList
   ArrayList如果构造函数没有传入容量参数，默认容量为10，使用懒加载过程，也就是第一次add时才会分配10个大小的Object数组。当加入元素个数大于10时(11个时)，会扩容到原来大小的1.5倍。扩容，是重新定义一个原来大小1.5倍大小的数组，将原来的数据复制到新数组中，再把对象指向新的数组地址即可。  
