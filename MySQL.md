...
- drop语句将表所占用的空间全释放掉,会删除表的结构和数据。

### 总结
1. 在速度上，一般来说，drop> truncate > delete
2. 如果想保留表而将所有数据删除，如果和事务无关，用truncate即可
...
### 为什么索引不选择红黑树，而选择b+树
 红黑树本质是二叉树，因为数据量大的时候，红黑树的树高很高，如果刚好查找叶子节点，那么就要经过多次io。而b+数树高很矮时，就可以存储大量的数据,并且所有叶子节点在同一层。   
#### 关于Hash索引    
...
## 7. MySQL锁  
![mysql锁.PNG](https://i.loli.net/2020/09/05/iN34MTHZCYv98QE.png)
要注意区分概念，乐观锁、悲观锁是mysql锁的实现方式，行锁和表锁是锁的粒度

### 乐观锁
用数据版本（Version）记录机制实现，这是乐观锁最常用的一种实现方式。何谓数据版本？即为数据增加一个版本标识，一般是通过为数据库表增加一个数字类型的 “version” 字段来实现。当读取数据时，将version字段的值一同读出，数据每更新一次，对此version值加1。当我们提交更新的时候，判断数据库表对应记录的当前版本信息与第一次取出来的version值进行比对，如果数据库表当前版本号与第一次取出来的version值相等，则予以更新，否则认为是过期数据。比如多版本并发控制**MVCC**
### 悲观锁
与乐观锁相对应的就是悲观锁了。悲观锁就是在操作数据时，认为此操作会出现数据冲突，所以在进行每次操作时都要通过获取锁才能进行对相同数据的操作，悲观锁是由数据库自己实现了的，要用的时候，我们直接调用数据库的相关语句就可以了。  

共享锁和排他锁都是悲观锁的不同实现，他们都是属于悲观锁的范围。
...
在执行语句后面加上 **for update** 就可添加排他锁。要使用排他锁，我们必须关闭**mysql数据库的自动提交属性，因为MySQL默认使用autocommit模式**，也就是说，当你执行一个更新操作后，MySQL会立刻将结果进行提交。


### 行锁

InnoDB只有在通过索引条件检索数据时使用行级锁，否则使用表锁！InnoDB的行锁是针对索引加的锁，不是针对记录加的锁。并且该索引不能失效，否则都会从行锁升级为表锁。

行锁的劣势：开销大；加锁慢；会出现死锁
行锁的优势：锁的粒度小，发生锁冲突的概率低；处理并发的能力强
加锁的方式：自动加锁。对于UPDATE、DELETE和INSERT语句，InnoDB会自动给涉及数据集加排他锁；对于普通SELECT语句，InnoDB不会加任何锁；当然我们也可以显示的加锁：
#### 间隙锁
当我们用**范围条件**而不是相等条件检索数据，并请求共享或排他锁时，InnoDB会给符合条件的已有数据记录的索引项加锁。这种锁机制就是所谓的**间隙锁(next-key锁)**，使用间隙锁可以解决幻读。  
### 表锁
innodb 的行锁是在有索引的情况下，没有索引的表是锁定全表的。**只有通过索引条件检索数据，InnoDB才使用行级锁，否则，InnoDB将使用表锁！**
...
## MyISAM存储引擎相关锁
- 共享读锁：对MyISAM表的读操作（加读锁），不会阻塞其他进程对同一表的读操作，但会阻塞对同一表的写操作。只有当读锁释放后，才能执行其他进程的写操作。在锁释放前不能读其他表。  
- 独占写锁：对MyISAM表的写操作（加写锁），会阻塞其他进程对同一表的读和写操作，只有当写锁释放后，才会执行其他进程的读写操作。在锁释放前不能写其他表。
MyISAM中写锁比读锁优先级更高，两个进程分别同时请求读锁和写锁，会先获取写锁。
### 7.3 MVCC  
数据库事务有不同的隔离级别，不同的隔离级别对锁的使用是不同的，锁的应用最终导致不同事务的隔离级别。  
MVCC(多版本并发控制)可以简单地认为：MVCC就是行级锁的一个变种(升级版)。在表锁中我们读写是阻塞的，基于提升并发性能的考虑，MVCC一般读写是不阻塞的。  
  
从用户的角度来看,**MVCC像是数据库可以提供同一数据的多个版本**。  
  
InnoDB的MVCC，是通过在每行记录后面保存两个隐藏的列来实现的。这两个列，一个保存了这条记录的**创建时间**，一个保存了这条记录的**删除时间**。存储的并不是实际的时间值，而是**系统版本号（system version number）**，每开始一个新的事务，系统版本号就会自动递增。事务开始时刻的系统版本号会作为事务的版本号，用来和查询到的每行记录的版本号进行比较。
  
  
对于SELECT查询语句,InnoDB会根据以下两个条件检查每条记录：  
- InnoDB只查找当前版本早于当前事务版本的数据行，这样可以确保事务读取的行，要么在事务开始前已经存在(行记录版本号小于事务版本号),要么事务自身插入或者修改过的(相等)。  
- 行的删除版本要么未定义，要么大于当前版本号。这样可以确保事务读取到的行，在事务开始之前未被删除。  
  
MVCC是在事务隔离级别的**读提交(read commited)** 和 **可重复读(Repeated read)**下工作的。

## 8. 数据库的三大范式
  
- **列不可分**：保证每列的属性不可再分.  
  
- **要有主键**：保证一张表只描述一件事情，主键列与非主键列遵循完全函数依赖关系  
  
- **不可传递依赖**：表中的字段和主键直接对应，不依赖其他中间字段，决定某字段的必须是主键。
